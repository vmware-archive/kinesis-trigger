...
func NewKinesisTriggerController(cfg KinesisTriggerConfig) *KinesisTriggerController {
	...
	kinesisInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			...
			if err == nil {
				newKinesisTriggerObj := new.(*kinesisApi.KinesisTrigger)
				oldKinesisTriggerObj := old.(*kinesisApi.KinesisTrigger)
	...
	controller := KinesisTriggerController{
		...
		kinesisClient:    cfg.TriggerClient,
...
func (c *KinesisTriggerController) processNextItem() bool {
	...
	if err == nil {
		...
		c.queue.Forget(key)
	} else if c.queue.NumRequeues(key) < kenisisTriggerMaxRetries {
...
func (c *KinesisTriggerController) syncKinesisTrigger(key string) error {
	...
	triggerObj := obj.(*kinesisApi.KinesisTrigger)
	...
	_, err = kubelessutils.GetFunctionCustomResource(c.kubelessClient, triggerObj.Spec.FunctionName, ns)
	if err != nil {
...
func (c *KinesisTriggerController) FunctionAddedDeletedUpdated(obj interface{}, deleted bool) error {
	...
	if deleted {
		...
		kinesisTriggersList, err := c.kinesisClient.KubelessV1beta1().KinesisTriggers(functionObj.Namespace).List(metav1.ListOptions{})
		if err != nil {
			return err
		}
		for _, kinesisTrigger := range kinesisTriggersList.Items {
			if kinesisTrigger.Spec.FunctionName == functionObj.Name {
				err = c.kinesisClient.KubelessV1beta1().KinesisTriggers(functionObj.Namespace).Delete(kinesisTrigger.Name, &metav1.DeleteOptions{})
...
func (c *KinesisTriggerController) kinesisTriggerObjNeedFinalizer(triggercObj *kinesisApi.KinesisTrigger) bool {
...
func (c *KinesisTriggerController) kinesisTriggerHasFinalizer(triggercObj *kinesisApi.KinesisTrigger) bool {
	currentFinalizers := triggercObj.ObjectMeta.Finalizers
	for _, f := range currentFinalizers {
		if f == kenisisTriggerFinalizer {
			return true
...
func (c *KinesisTriggerController) kinesisTriggerObjAddFinalizer(triggercObj *kinesisApi.KinesisTrigger) error {
	...
	return utils.UpdateKinesisTriggerCustomResource(c.kinesisClient, triggercObjClone)
}

func (c *KinesisTriggerController) kinesisTriggerObjRemoveFinalizer(triggercObj *kinesisApi.KinesisTrigger) error {
	triggercObjClone := triggercObj.DeepCopy()
	...
	err := utils.UpdateKinesisTriggerCustomResource(c.kinesisClient, triggercObjClone)
	if err != nil {
		return err
	}
	return nil
...
func kinesisTriggerObjChanged(oldKinesisTriggerObj, newKenisisTriggerObj *kinesisApi.KinesisTrigger) bool {
