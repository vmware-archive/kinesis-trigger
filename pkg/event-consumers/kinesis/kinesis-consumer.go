...
func createStreamProcessor(triggerObj *kinesisApi.KinesisTrigger, funcName, ns string, clientset kubernetes.Interface, stopchan, stoppedchan chan struct{}) {
...
func CreateKinesisStreamConsumer(triggerObj *kinesisApi.KinesisTrigger, funcName, ns string, clientset kubernetes.Interface) error {
	mutex.Lock()
	defer mutex.Unlock()
...
func DeleteKinesisConsumer(triggerObj *kinesisApi.KinesisTrigger, funcName, ns string) error {
	mutex.Lock()
	defer mutex.Unlock()
	uniqueID := generateUniqueStreamProcessorID(triggerObj.Name, funcName, ns, triggerObj.Spec.Stream)
	if streamProcessors[uniqueID] {
